...
export default async function EmpleosPage() {
  ...
  const [{ data: jobs }, organization] = await Promise.all([
    supabase
      .from("jobs")
  ...
  return (
    <div className="mx-auto max-w-4xl px-6 py-16">
      <h1 className="font-serif text-[40px]">{organization?.careers_headline || "Vacantes abiertas"}</h1>
      {organization?.careers_intro && (
        <p className="mt-4 max-w-2xl whitespace-pre-wrap text-[15px] leading-relaxed text-muted-foreground">{organization.careers_intro}</p>
      )}
      {!jobs || jobs.length === 0 ? (
