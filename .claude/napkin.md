# Napkin Runbook — ATS
_Última actualización: 2026-09-03 (flujo de solicitud de vacante: plantilla general, estado aceptada, visibilidad de 3 niveles)_
...
7. **[2026-09-03] `job_collaborators` en `createJob` se arma con un `Map<profileId, permission>` insertado de menor a mayor nivel (viewer → approver → owner), no un array plano — así, si la misma persona cae en dos baldes a la vez (ej. el solicitante también aparece como colaborador adicional por error del formulario), gana el nivel más alto sin violar el `UNIQUE(job_id, profile_id)`.** Patrón reusable: cuando varias fuentes pueden nombrar a la misma persona con distinto nivel, un Map con inserción ordenada de menor a mayor es más simple que deduplicar un array después.
8. **Pendiente, a propósito, no por olvido — 3 cosas que siguen abiertas:** (a) `canDecideApplication`/`canRateApplication` (`src/lib/applications/permissions.ts`) todavía no aplican los niveles de `job_collaborators` a `gestor` — hoy cualquier rol que no sea `colaborador` pasa de largo, y con `colaborador` vacío esa excepción es universal; no rompe nada NUEVO (mismo comportamiento que antes para gestor/admin), pero tampoco cumple la decisión del usuario todavía. (b) Eliminar de verdad el rol `colaborador` de la interfaz (quitarlo de `invite-form.tsx`, limpiar los ~11 puntos que preguntan "¿es colaborador?") — declarado "el paso más delicado, va solo" desde el plan original, sigue sin tocarse. (c) Filtro de plantillas por área del solicitante — bloqueado en datos reales del cliente (`profiles.department_id` sin poblar).
