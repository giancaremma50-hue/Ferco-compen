# Napkin Runbook — ATS
_Última actualización: 2026-09-03 (permisos de 2 niveles por vacante, competencias eliminadas, agenda protagonista en Inicio)_
...
8. **Pendiente, a propósito, no por olvido:** (a) ~~rewire de `canDecideApplication`~~ — hecho, ver la sección de arriba; ese pendiente resultó ser un agujero de seguridad activo, no una deuda cosmética. (b) Eliminar de verdad el rol `colaborador` de la interfaz (quitarlo de `invite-form.tsx`, limpiar los puntos que preguntan "¿es colaborador?") — declarado "el paso más delicado, va solo" desde el plan original, sigue sin tocarse; mientras siga invitable, las guardias de `createJob` y `/vacantes/nueva` se quedan. (c) Filtro de plantillas por área del solicitante — bloqueado en datos reales del cliente (`profiles.department_id` sin poblar).
